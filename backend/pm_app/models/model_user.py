...
class UserManager(BaseUserManager):
    def _create_user(self, username, email=None, password=None, is_staff, is_superuser, **extra_fields):
        now = timezone.now()
        if not username:
    ...
    def create_superuser(self, username, email, password, **extra_fields):
        user= self.create_user(username, email, password, True, True, **extra_fields)
        user.is_active=True 
        user.save(using=self._db)
        return user
...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    username= models.CharField(_('username'), max_length=15,
                               unique=True,
                               help_text=_('Required. 15 characteres or fewer. Letters, \
                                           number and @/./+/-/_ characters'),
                                           validators=[
